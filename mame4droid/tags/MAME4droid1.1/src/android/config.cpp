...
void get_rom_sample_path (int argc, char **argv, int game_index)
{
	...
	static char samplepath_buf[512];
	...
	sprintf(rompath_buf,"%s.;%sroms",globalpath,globalpath);
	sprintf(samplepath_buf,"%s.;%ssamples",globalpath,globalpath);

	rompath    = get_string ("directory", "rompath",    NULL, rompath_buf);
	samplepath = get_string ("directory", "samplepath", NULL, samplepath_buf);
...
{
	static char nvdir_buf[256];
	static char hidir_buf[256];
	static char cfgdir_buf[256];
	static char screenshotdir_buf[256];
	static char memcarddir_buf[256];
	static char stadir_buf[256];
	static char artworkdir_buf[256];
	static char cheatdir_buf[256];
	...
	sprintf(nvdir_buf,"%snvram",globalpath);
	sprintf(hidir_buf,"%shi",globalpath);
	sprintf(cfgdir_buf,"%scfg",globalpath);
	sprintf(screenshotdir_buf,"%ssnap",globalpath);
	...
	hidir      = get_string ("directory", "hi",      NULL, hidir_buf);
	cfgdir     = get_string ("directory", "cfg",     NULL, cfgdir_buf);
	screenshotdir = get_string ("directory", "snap",     NULL, screenshotdir_buf);
	memcarddir = get_string ("directory", "memcard", NULL, memcarddir_buf);
	stadir     = get_string ("directory", "sta",     NULL, stadir_buf);
	artworkdir = get_string ("directory", "artwork", NULL, artworkdir_buf);
	cheatdir = get_string ("directory", "cheat", NULL, cheatdir_buf);
